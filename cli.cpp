...
void print_usage() {
    std::cerr << "\033[1;32mhuffman\033[0m - something about data compression\n\n"
              ...
              << "  -d, --decompress\n"
              << "  -h, --help (default)\n\n"
              << "If only output file is not specified, output is written to <input file>.out.\n"
              << "If both files are not specified, input is STDIN and output is STDOUT." << std::endl;
}

int process(huffman_mode_t mode, std::istream& in, std::ostream& out) {
    try {
        clock_t begin = clock();
...
int main(int argc, char *argv[]) {
    ...
    if (argc == 2) {
        return process(mode, std::cin, std::cout);
    } else {
        ...
        try {
            ...
            return process(mode, input_file, output_file);
        } catch (std::exception &e) {
            std::cerr << "\033[1;31m[ERROR]\033[0m " << e.what() << std::endl;
            return 2;
        }
